...
{
public:
    AnglesConverter() : Node("angles_converison_service")
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Angle conversion service is Ready");
    }
...
private:
    ...
    { 
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Request to convert euler angles roll: " << req->roll << " pitch: " << req->pitch << " yaw: " << req->yaw << " into quaternion");
        tf2::Quaternion q;
    ...
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Request to convert quaternion x: " << req->x << " y: " << req->y << " z: " << req->z << " w: " << req->w << " into euler");
        tf2::Quaternion q(req->x, req->y, req->z, req->w);
        tf2::Matrix3x3 m(q);
        m.getRPY(res->roll, res->pitch, res->yaw);
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Corresponding euler roll: " << res->roll << " pitch: " << res->pitch << " yaw: " << res->yaw);
    }
};
