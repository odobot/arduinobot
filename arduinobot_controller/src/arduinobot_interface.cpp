...
namespace arduinobot_controller
{
    ...
    CallbackReturn ArduinobotInterface::on_activate(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Failed to inrterface with the port " << port_);
            return CallbackReturn::FAILURE;
        }
    ...
    CallbackReturn ArduinobotInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        if (arduino_.IsOpen())
        {
            ...
            {
                RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Failed to close the connection with the port " << port_);
                return CallbackReturn::FAILURE;
            }
        }
    ...
    hardware_interface::return_type ArduinobotInterface::write(const rclcpp::Time &time, const rclcpp::Duration &period)
    {
        ...
        {
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something wen wrong while sending the message " << msg);
